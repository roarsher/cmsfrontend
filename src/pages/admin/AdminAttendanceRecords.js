...
// ── PDF Generator ─────────────────────────────────────────────────────────────
// Now includes: Course name · Department · Semester · Teacher
// Students shown are ONLY those enrolled in that course (fixed on backend)
const downloadSessionPDF = (session) => {
  ...
  const rows = entries.map((e, i) => {
    ...
    return `<tr style="background:${i % 2 === 0 ? "#f8fafc" : "#fff"}">
      ...
      <td style="padding:10px 16px;border-bottom:1px solid #e2e8f0;color:${color};font-weight:700;font-size:13px">${statusLabel}</td>
  ...
  // Colour the pct bar green/amber/red
  const barColor = pct >= 75 ? "#10b981" : pct >= 50 ? "#f59e0b" : "#ef4444";

  const html = `<!DOCTYPE html>
<html><head><title>Attendance Report — ${session.course?.name || ""}</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .pct-bar-bg{background:#e2e8f0;border-radius:999px;height:12px;overflow:hidden}
  .pct-bar-fill{height:100%;border-radius:999px}
  .pct-label{display:flex;justify-content:space-between;font-size:13px;font-weight:700;margin-bottom:6px}
  /* ── Table ── */
  ...
  <!-- Course banner — shows what course+branch+sem this report covers -->
  <div class="course-banner">
    <div>
      <div class="label">Course</div>
      <div class="value">${session.course?.name || "—"} &nbsp;(${session.course?.code || ""})</div>
    </div>
    <div>
      <div class="label">Department &amp; Semester</div>
      <div class="value">${session.department || "—"} · Semester ${session.semester || "—"}</div>
    </div>
    <div>
      <div class="label">Teacher</div>
      <div class="value">${session.teacher?.name || "—"}</div>
    </div>
    <div>
      <div class="label">Enrolled Students</div>
      <div class="value">${r.totalStudents || 0} students</div>
    </div>
  </div>
  ...
  <table>
    ...
    <tbody>
      ${rows || `<tr><td colspan="6" style="text-align:center;padding:24px;color:#94a3b8">No records yet</td></tr>`}
    </tbody>
  </table>

  <div class="footer">
    <p>Generated on ${new Date().toLocaleString("en-IN")} &nbsp;·&nbsp; BCE Bhagalpur College ERP</p>
    <p style="margin-top:4px">Only students enrolled in <strong>${session.course?.name || "this course"}</strong>
       (${session.department} — Semester ${session.semester}) are listed.</p>
    <p style="margin-top:4px">This is a computer-generated report. No signature required.</p>
  </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Course info banner */}
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 grid grid-cols-2 gap-3 text-sm">
            <div>
              <p className="text-xs text-blue-400 font-semibold uppercase">Course</p>
              <p className="font-bold text-blue-800">{session.course?.name} ({session.course?.code})</p>
            </div>
            <div>
              <p className="text-xs text-blue-400 font-semibold uppercase">Scope</p>
              <p className="font-bold text-blue-800">{session.department} · Semester {session.semester}</p>
            </div>
            <div>
              <p className="text-xs text-blue-400 font-semibold uppercase">Teacher</p>
              <p className="font-bold text-blue-800">{session.teacher?.name || "—"}</p>
            </div>
            <div>
              <p className="text-xs text-blue-400 font-semibold uppercase">Note</p>
              <p className="text-blue-700 text-xs">Only course-enrolled students listed</p>
            </div>
          </div>
...
const AdminAttendanceRecords = () => {
  ...
  const filtered = sessions.filter((s) => {
    ...
    return matchDept && matchSem && matchDate && matchSearch;
  });
    ...
    <div className="min-h-screen bg-slate-50 p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          All live attendance sessions — filtered by <strong>course → department → semester</strong>
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex flex-wrap gap-3">
        ...
        <input
          ...
          className="border border-slate-200 rounded-xl px-3 py-2 text-sm outline-none"
        />
        {(filterDept || filterSem || filterDate || search) && (
          <button
            onClick={() => { setFilterDept(""); setFilterSem(""); setFilterDate(""); setSearch(""); }}
        ...
        <div className="p-5 border-b border-slate-100">
          <h2 className="font-bold text-slate-700">
            ...
            <span className="text-xs font-normal text-slate-400 ml-2">
              — each report lists only students enrolled in that course
            </span>
          </h2>
        </div>
            ...
            <tbody className="divide-y divide-slate-50">
              ...
              {filtered.map((s, i) => {
                const pct = s.report?.totalStudents > 0
                  ? Math.round((s.report.presentCount / s.report.totalStudents) * 100)
                  : 0;
