...
const Toast = ({ msg, type, onClose }) => {
  ...
  const colors = { success: "bg-emerald-500", error: "bg-red-500", info: "bg-blue-500" };
  return (
    <div className={`fixed top-5 right-5 z-[999] flex items-center gap-3 px-5 py-3 rounded-2xl shadow-xl text-white text-sm font-medium ${colors[type] || colors.info}`}>
      {type === "success" && <Icons.Check />}
      {type === "error"   && <Icons.Alert />}
      <span>{msg}</span>
      <button onClick={onClose} className="ml-2 opacity-70 hover:opacity-100"><Icons.Close /></button>
...
const FormInput    = ({ label, ...props }) => (
  <div className="flex flex-col gap-1">
    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{label}</label>
    <input className="border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 bg-gray-50 transition" {...props} />
  </div>
);
const FormSelect   = ({ label, children, ...props }) => (
  <div className="flex flex-col gap-1">
    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{label}</label>
    <select className="border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 bg-gray-50 transition" {...props}>{children}</select>
  </div>
);
const FormTextArea = ({ label, ...props }) => (
  <div className="flex flex-col gap-1">
    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{label}</label>
    <textarea rows={4} className="border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 bg-gray-50 transition resize-none" {...props} />
  </div>
);
const SubmitBtn    = ({ loading, children, onClick }) => (
...
const SemesterPills = ({ value, onChange, label = "Semester" }) => (
...
// ─────────────────────────────────────────────────────────────────────────────
// MARK ATTENDANCE MODAL  (now with semester selector)
// ─────────────────────────────────────────────────────────────────────────────
const AttendanceModal = ({ allStudents, courses, onClose, onToast, onRefresh }) => {
  ...
  // Filter students by chosen semester
  const semStudents = allStudents.filter((s) => s.semester === selectedSem);

  const [rows, setRows] = useState([]);

  // Re-build rows whenever semester changes
  useEffect(() => {
    setRows(
      allStudents
        .filter((s) => s.semester === selectedSem)
        .map((s) => ({ studentId: s._id, name: s.name, roll: s.rollNumber, status: "Present" }))
    );
  }, [selectedSem, allStudents]);

  // Filter courses by semester too
  ...
  const toggle       = (id) => setRows((prev) => prev.map((r) => r.studentId === id ? { ...r, status: r.status === "Present" ? "Absent" : "Present" } : r));
  ...
  const submit = async () => {
    if (!courseId)       return onToast("Please select a course", "error");
    if (rows.length === 0) return onToast(`No students in Semester ${selectedSem}`, "error");
    setLoading(true);
    try {
      await Promise.all(rows.map((r) => API.post("/attendance/mark", { studentId: r.studentId, courseId, status: r.status })));
      onToast(`Attendance marked for Sem ${selectedSem}! ${presentCount}P / ${absentCount}A`, "success");
      onRefresh();
      onClose();
    } catch (err) {
      onToast(err.response?.data?.message || "Failed to mark attendance", "error");
    ...
    <Modal title="✅ Mark Attendance" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4">
          <SemesterPills value={selectedSem} onChange={setSelectedSem} label="Select Semester" />
          <p className="text-xs text-blue-600 mt-2 font-medium">
            {semStudents.length} student{semStudents.length !== 1 ? "s" : ""} in Semester {selectedSem}
          </p>
        </div>

        {/* Course selector — only courses of chosen semester */}
        <FormSelect label={`Course for Semester ${selectedSem}`} value={courseId} onChange={(e) => setCourseId(e.target.value)}>
          {semCourses.length === 0
            ? <option value="">No courses found for Sem {selectedSem}</option>
            : semCourses.map((c) => <option key={c._id} value={c._id}>{c.name} ({c.code})</option>)
          }
        </FormSelect>

        {/* Bulk buttons */}
        <div className="flex gap-2">
          <button onClick={() => markAll("Present")} className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold py-2 rounded-xl transition">✓ All Present</button>
          <button onClick={() => markAll("Absent")}  className="flex-1 bg-red-500    hover:bg-red-600    text-white text-xs font-bold py-2 rounded-xl transition">✗ All Absent</button>
        </div>

        <p className="text-xs text-gray-400 bg-blue-50 rounded-xl px-3 py-2 text-center">
          💡 Mark all present first, then toggle absent students.
        </p>
        ...
        <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
          {rows.length === 0 && (
            <p className="text-center text-gray-400 text-sm py-6">No students in Semester {selectedSem}</p>
          )}
          {filtered.map((r) => (
          ...
          {filtered.length === 0 && rows.length > 0 && <p className="text-center text-gray-400 text-sm py-4">No students found</p>}
        </div>
        ...
        <SubmitBtn loading={loading} onClick={submit}>
          Submit Attendance — Sem {selectedSem} ({presentCount}P / {absentCount}A)
        </SubmitBtn>
      </div>
    </Modal>
  );
...
// ─────────────────────────────────────────────────────────────────────────────
// ADD MARKS MODAL  (semester-aware student search)
// ─────────────────────────────────────────────────────────────────────────────
const MarksModal = ({ allStudents, courses, onClose, onToast, onRefresh }) => {
  const [selectedSem, setSelectedSem]   = useState(1);
  const [form, setForm]                 = useState({ search: "", studentId: "", courseId: "", marks: "" });
  const [suggestions, setSuggestions]   = useState([]);
  const [selectedStudent, setSelectedStudent] = useState(null);
  const [loading, setLoading]           = useState(false);
  ...
  useEffect(() => {
    setForm({ search: "", studentId: "", courseId: semCourses[0]?._id || "", marks: "" });
    setSelectedStudent(null);
    setSuggestions([]);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedSem]);
  ...
  const handleSearch = (q) => {
    ...
    setSelectedStudent(null);
    if (q.length < 2) { setSuggestions([]); return; }
    // Filter locally from allStudents, restricted to selected semester
  ...
  const submit = async () => {
    ...
    if (!form.courseId)  return onToast("Please select a course",  "error");
    if (form.marks === "" || isNaN(form.marks)) return onToast("Enter valid marks (0–100)", "error");
    if (Number(form.marks) < 0 || Number(form.marks) > 100) return onToast("Marks must be 0–100", "error");
    setLoading(true);
    try {
      await API.post("/teacher/marks", { studentId: form.studentId, courseId: form.courseId, marks: Number(form.marks) });
      onToast("Marks added successfully!", "success");
      onRefresh();
      onClose();
    } catch (err) {
      ...
      <div className="space-y-4">
        ...
        <div className="bg-violet-50 border border-violet-100 rounded-2xl p-4">
          ...
          <p className="text-xs text-violet-600 mt-2 font-medium">
            Showing students &amp; courses for Semester {selectedSem}
          </p>
        </div>

        {/* Student search — only within selected semester */}
        <div className="relative">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
              Search Student — Semester {selectedSem}
            </label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><Icons.Search /></span>
              <input placeholder="Type name or roll number…" value={form.search} onChange={(e) => handleSearch(e.target.value)}
                className="w-full border border-gray-200 rounded-xl pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300 bg-gray-50" />
            </div>
          </div>
          {suggestions.length > 0 && (
            <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-xl shadow-lg mt-1 max-h-48 overflow-y-auto">
              {suggestions.map((s) => (
                <button key={s._id} type="button" onClick={() => selectStudent(s)}
                  className="w-full text-left px-4 py-3 hover:bg-blue-50 transition border-b border-gray-50 last:border-0">
                  <p className="text-sm font-semibold text-gray-800">{s.name}</p>
                  <p className="text-xs text-gray-400">Roll: {s.rollNumber} · Sem {s.semester} · Year {s.year}</p>
                </button>
              ))}
            </div>
          )}
        ...
        {selectedStudent && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 text-sm">
            <p className="font-semibold text-blue-800">{selectedStudent.name}</p>
            <p className="text-blue-600 text-xs mt-0.5">
              Roll: {selectedStudent.rollNumber} · Sem {selectedStudent.semester} · Year {selectedStudent.year} · {selectedStudent.email}
            </p>
          </div>
        )}

        {/* Course selector — only semester's courses */}
        <FormSelect label={`Course — Semester ${selectedSem}`}
          value={form.courseId}
          onChange={(e) => setForm((f) => ({ ...f, courseId: e.target.value }))}>
          {semCourses.length === 0
            ? <option value="">No courses for Sem {selectedSem}</option>
            : semCourses.map((c) => <option key={c._id} value={c._id}>{c.name} ({c.code})</option>)
          }
        </FormSelect>
...
const TopPerformersModal = ({ students, onClose }) => {
  ...
  const pool   = filterSem === "all" ? students : students.filter((s) => s.semester === Number(filterSem));
    ...
    <Modal title="🏆 Top Performers" onClose={onClose}>
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2 items-center">
          <button onClick={() => setFilterSem("all")}
            className={`px-3 py-1 rounded-full text-xs font-bold border transition ${filterSem === "all" ? "bg-blue-600 text-white border-blue-600" : "bg-gray-100 text-gray-500 border-gray-200"}`}>
            All Sems
          </button>
          {SEMESTERS.map((s) => (
            <button key={s} onClick={() => setFilterSem(String(s))}
              className={`px-3 py-1 rounded-full text-xs font-bold border transition ${filterSem === String(s) ? "bg-blue-600 text-white border-blue-600" : "bg-gray-100 text-gray-500 border-gray-200"}`}>
              Sem {s}
            </button>
          ))}
        </div>
          ...
          {sorted.map((s, i) => {
            ...
            return (
              <div key={s._id} className={`flex items-center gap-4 p-4 rounded-2xl border ${i < 3 ? "border-yellow-200 bg-yellow-50" : "border-gray-100 bg-gray-50"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-gray-800 truncate">{s.name}</p>
                  <p className="text-xs text-gray-400">Roll: {s.rollNumber} · Sem {s.semester}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-violet-700">{s.avgMarks ?? "—"}<span className="text-xs text-gray-400">/100</span></p>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${col.bg} ${col.text}`}>{s.attendance}% att.</span>
...
const TeacherDashboard = () => {
  ...
  // All students (full department) fetched once
  ...
  // ── Semester filter for the student table ─────────────────────────────────
  const [activeSem, setActiveSem]   = useState("all"); // "all" | 1..8
  const [activeTab, setActiveTab]   = useState("all"); // "all" | "atrisk" | "toppers"
  const [searchQuery, setSearchQuery] = useState("");
  ...
  const fetchData = useCallback(async () => {
    try {
      const [studRes, courseRes, noticeRes, classRes] = await Promise.allSettled([
        API.get("/teacher/students"),        // returns ALL dept students
        API.get("/teacher/courses"),
        API.get("/teacher/notices"),
        API.get("/routines/my-today"),
      ]);
      ...
      if (studRes.status === "fulfilled") {
        setAllStudents(studRes.value.data.students || []);
      } else {
        // Demo fallback
        setAllStudents([
          { _id:"1", name:"Ritu Sharma",    rollNumber:"CSE225001", department:"CSE", semester:3, year:2, attendance:92, avgMarks:87, email:"ritu@college.edu" },
          { _id:"2", name:"Pawan Kumar",    rollNumber:"CSE225002", department:"CSE", semester:3, year:2, attendance:68, avgMarks:54, email:"pawan@college.edu" },
          { _id:"3", name:"Abhishek Singh", rollNumber:"CSE225003", department:"CSE", semester:5, year:3, attendance:80, avgMarks:73, email:"abhi@college.edu" },
          { _id:"4", name:"Niranjan Das",   rollNumber:"CSE225004", department:"CSE", semester:1, year:1, attendance:55, avgMarks:45, email:"niranjan@college.edu" },
          { _id:"5", name:"Pooja Verma",    rollNumber:"CSE225005", department:"CSE", semester:5, year:3, attendance:88, avgMarks:91, email:"pooja@college.edu" },
          { _id:"6", name:"Rahul Mehta",    rollNumber:"CSE225006", department:"CSE", semester:7, year:4, attendance:76, avgMarks:68, email:"rahul@college.edu" },
        ]);
      }
      ...
      if (courseRes.status === "fulfilled") {
        ...
        setCourses([
          { _id:"c1", name:"Data Structures",  code:"CSE-S3-01", semester:3 },
          { _id:"c2", name:"Operating Systems", code:"CSE-S4-01", semester:4 },
          { _id:"c3", name:"DBMS",              code:"CSE-S5-01", semester:5 },
        ]);
      }
  ...
  // ── Students visible in the table (filtered by semester + tab + search) ───
  const semFiltered = activeSem === "all"
    ? allStudents
    : allStudents.filter((s) => s.semester === Number(activeSem));

  const displayed = semFiltered
    .sort((a, b) => (a.attendance || 0) - (b.attendance || 0))
    .filter((s) => {
      const q = searchQuery.toLowerCase();
  ...
  // ── Stats (based on current semester filter) ──────────────────────────────
  const statBase      = semFiltered;
  const totalStudents = statBase.length;
  const avgAttendance = totalStudents ? Math.round(statBase.reduce((s, st) => s + (st.attendance || 0), 0) / totalStudents) : 0;
  const avgMarks      = totalStudents ? Math.round(statBase.reduce((s, st) => s + (st.avgMarks || 0), 0) / totalStudents) : 0;
  const atRisk        = statBase.filter((s) => (s.attendance || 0) < 75).length;

  const barData = statBase.map((s) => ({ name: s.name?.split(" ")[0], Attendance: s.attendance || 0, Marks: s.avgMarks || 0 }));
  const pieData = [
    { name: "≥ 85%",  value: statBase.filter((s) => (s.attendance || 0) >= 85).length },
    { name: "75–84%", value: statBase.filter((s) => (s.attendance || 0) >= 75 && (s.attendance || 0) < 85).length },
    { name: "< 75%",  value: statBase.filter((s) => (s.attendance || 0) < 75).length },
  ];

  const toppers = [...statBase].sort((a, b) => (b.avgMarks || 0) - (a.avgMarks || 0)).slice(0, 3);
  ...
  const sidebarNotices = notices.length > 0
    ...
    : [
        { text: `${atRisk} student(s) below 75% attendance`, time: "Today", type: atRisk > 0 ? "alert" : "success" },
        { text: "Mid-semester exams scheduled next week",     time: "Yesterday", type: "info" },
        { text: "Grade submission deadline: 15th",           time: "2 days ago", type: "alert" },
      ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 space-y-6 text-gray-800">
      ...
      {modal === "attendance" && (
        <AttendanceModal allStudents={allStudents} courses={courses} onClose={() => setModal(null)} onToast={showToast} onRefresh={fetchData} />
      )}
      {modal === "marks" && (
        <MarksModal allStudents={allStudents} courses={courses} onClose={() => setModal(null)} onToast={showToast} onRefresh={fetchData} />
      )}
      {modal === "notice" && (
        <NoticeModal onClose={() => setModal(null)} onToast={showToast} onRefresh={fetchData} />
      )}
      {modal === "toppers" && (
        <TopPerformersModal students={allStudents} onClose={() => setModal(null)} />
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Dept: <span className="font-semibold text-blue-500">{user?.department || "—"}</span>
            &nbsp;·&nbsp;
            {new Date().toLocaleDateString("en-IN", { weekday:"long", year:"numeric", month:"long", day:"numeric" })}
          </p>
        </div>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
        <div className="flex flex-wrap items-center gap-3">
          <span className="text-sm font-bold text-gray-600">📚 View Semester:</span>
          <button
            onClick={() => setActiveSem("all")}
            className={`px-4 py-1.5 rounded-full text-xs font-bold border transition ${activeSem === "all" ? "bg-blue-600 text-white border-blue-600 shadow" : "bg-gray-100 text-gray-500 border-gray-200 hover:border-blue-300"}`}>
            All Semesters
          </button>
          {SEMESTERS.map((s) => {
            const count = allStudents.filter((st) => st.semester === s).length;
            return (
              <button key={s} onClick={() => setActiveSem(s)}
                className={`relative px-4 py-1.5 rounded-full text-xs font-bold border transition ${activeSem === s ? "bg-blue-600 text-white border-blue-600 shadow" : "bg-gray-100 text-gray-500 border-gray-200 hover:border-blue-300"}`}>
                Sem {s}
                {count > 0 && (
                  <span className={`ml-1.5 inline-flex items-center justify-center w-4 h-4 rounded-full text-[10px] font-black ${activeSem === s ? "bg-white text-blue-600" : "bg-blue-100 text-blue-600"}`}>
                    {count}
                  </span>
                )}
              </button>
        ...
        {activeSem !== "all" && (
          <p className="text-xs text-blue-600 mt-3 font-medium bg-blue-50 rounded-xl px-3 py-2">
            📊 Showing stats &amp; students for <strong>Semester {activeSem}</strong> — {totalStudents} student{totalStudents !== 1 ? "s" : ""}
          </p>
        )}
      </div>
      ...
      {/* Stat Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Icons.Students}   label="Students"       value={totalStudents}       sub={activeSem === "all" ? "Full department" : `Semester ${activeSem}`} accent="bg-blue-500"    />
        <StatCard icon={Icons.Attendance} label="Avg Attendance"  value={`${avgAttendance}%`} sub="Class average"    accent="bg-emerald-500" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          ...
          <p className="text-xs text-gray-400 mb-4">{activeSem === "all" ? "All semesters" : `Semester ${activeSem} only`}</p>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={barData} barGap={4}>
              <XAxis dataKey="name" tick={{ fontSize: 12 }} />
              <YAxis domain={[0, 100]} tick={{ fontSize: 12 }} />
          ...
          <div className="p-5 border-b border-gray-100 space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <h2 className="text-lg font-bold text-gray-700 flex-1">
                👨‍🎓 Students
                {activeSem !== "all" && <span className="ml-2 text-sm text-blue-500 font-semibold">— Sem {activeSem}</span>}
              </h2>
                ...
                {displayed.length === 0 ? (
                  ...
                  return (
                    <tr key={s._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3 text-center">
                        <span className="bg-blue-100 text-blue-700 text-xs font-bold px-2 py-0.5 rounded-full">S{s.semester}</span>
                      </td>
                      <td className="px-5 py-3">
                        <div className="flex flex-col items-center gap-1">
                          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${col.bg} ${col.text}`}>{att}%</span>
            ...
            <div className="space-y-3">
              {toppers.map((s, i) => (
                <div key={s._id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800 truncate">{s.name}</p>
                    <p className="text-xs text-gray-400">Sem {s.semester} · {s.avgMarks ?? "—"} marks</p>
                  </div>
                </div>
              ))}
              {toppers.length === 0 && <p className="text-sm text-gray-400">No data yet</p>}
