...
//           return;
//         }
//         payload.departments     = formData.departments; // ✅ correct key
//         payload.designation     = formData.designation;
//         payload.TeacherIdNumber = formData.TeacherIdNumber;
...
//             <div className="space-y-2">
//               <label className="text-gray-400 text-xs pl-1">
//                 Departments (select 1–5) —{" "}
//                 <span className="text-yellow-400">{formData.departments.length} selected</span>
//               </label>
//               <div className="flex flex-wrap gap-2">
//                 {DEPARTMENTS.map((dept) => {
//                   const selected = formData.departments.includes(dept);
//                   const maxed    = !selected && formData.departments.length >= 5;
//                   return (
//                     <button
//                       key={dept}
//                       type="button"
...
const SignupForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    password:        "",
    department:      "",
    departments:     [], // ✅ for teacher multi-select
    enrollmentYear:  "",
    semester:        "",
    rollSerial:      "",
    regSerial:       "",
  ...
  const derivedYear = formData.semester ? getYearFromSemester(formData.semester) : "";
  const rollPrefix  = formData.enrollmentYear && formData.department
    ? buildRollPrefix(formData.enrollmentYear, formData.department) : "";
  const regPrefix   = formData.enrollmentYear && formData.department
    ? buildRegPrefix(formData.enrollmentYear, formData.department) : "";
  ...
  function switchRole(role) {
    ...
    setFormData((prev) => ({
      ...prev,
      department:      "",
      departments:     [], // ✅ reset array on role switch
      enrollmentYear:  "",
      semester:        "",
      rollSerial:      "",
      regSerial:       "",
  ...
  async function submitHandler(e) {
    ...
    try {
      ...
      // ── TEACHER ──────────────────────────────────────────────────
        ...
        payload.departments     = formData.departments; // ✅ correct key + correct value
        payload.designation     = formData.designation;
        payload.TeacherIdNumber = formData.TeacherIdNumber;
      ...
      // ── ADMIN ────────────────────────────────────────────────────
        ...
        payload.adminKey = formData.adminKey; // ✅ clean, no extra fields
      }
    ...
    <div className="w-full space-y-6">
      ...
      <form onSubmit={submitHandler} className="space-y-4">
        ...
        <input type="text"     name="name"     required placeholder="Enter Full Name"
          value={formData.name}     onChange={changeHandler} className={inputClass} />
        <input type="email"    name="email"    required placeholder="Enter Email Address"
          value={formData.email}    onChange={changeHandler} className={inputClass} />
        <input type="password" name="password" required placeholder="Create Password"
          value={formData.password} onChange={changeHandler} className={inputClass} />
          ...
          <>
            <div className="relative">
              <select name="department" required value={formData.department}
                onChange={changeHandler} className={selectClass}>
                <option value="" disabled>Select Department</option>
                {DEPARTMENTS.map((dept) => (
                  <option key={dept} value={dept}>{dept}</option>
                ))}
            ...
            <input
              type="number" name="enrollmentYear" required
              placeholder="Enrollment Year (e.g. 2022)"
              min="2000" max={new Date().getFullYear()}
              value={formData.enrollmentYear}
              onChange={changeHandler} className={inputClass}
            />

            <div className="relative">
              <select name="semester" required value={formData.semester}
                onChange={changeHandler} className={selectClass}>
                <option value="" disabled>Select Semester</option>
                {[1, 2, 3, 4, 5, 6, 7, 8].map((s) => (
                  <option key={s} value={s}>Semester {s}</option>
            ...
            <div className="space-y-1">
              <label className="text-gray-400 text-xs pl-1">Roll Number — enter your serial (1–999)</label>
              <PrefixInput
                prefix={rollPrefix} name="rollSerial"
                value={formData.rollSerial} onChange={changeHandler}
                placeholder="e.g. 36" padTo={null}
              />
            </div>
            ...
            <div className="space-y-1">
              <label className="text-gray-400 text-xs pl-1">Registration Number — enter your serial (1–999)</label>
              <PrefixInput
                prefix={regPrefix} name="regSerial"
                value={formData.regSerial} onChange={changeHandler}
                placeholder="e.g. 21" padTo={3}
              />
            </div>
          </>
        )}
        ...
        {accountType === "teacher" && (
          <>
            <input
              type="text" name="TeacherIdNumber" required
              placeholder="Teacher ID Number"
              value={formData.TeacherIdNumber}
              onChange={changeHandler} className={inputClass}
            />
